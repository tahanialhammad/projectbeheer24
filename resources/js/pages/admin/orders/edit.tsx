...
export default function Edit({ order }: { order: Order }) {
    ...
    return (
        <AppLayout>
            ...
            <form onSubmit={handleSubmit} className="max-w-xl space-y-6 rounded bg-white p-6 shadow-md">
                <div>
                    <label className="mb-1 block font-medium">Status</label>
                    <select value={data.status} onChange={(e) => setData('status', e.target.value)} className="w-full rounded border-neutral-300">
